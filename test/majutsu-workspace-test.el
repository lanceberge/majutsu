...
The Emacs-facing path remains unchanged for visiting the new workspace."
  (let (seen-args seen-visit seen-mkdir)
    (cl-letf (((symbol-function 'majutsu-convert-filename-for-jj)
      ...
      (should (equal seen-args '("workspace" "add" "/tmp/feature" "--name" "feature")))
      (should (equal seen-visit (expand-file-name "/ssh:demo:/tmp/feature"))))))

(ert-deftest majutsu-workspace--name-from-destination/uses-final-path-part ()
  "Workspace add names should come from the destination basename."
  (should (equal (majutsu-workspace--name-from-destination "~/jj-workspaces/repo/feature")
                 "feature"))
  (should (equal (majutsu-workspace--name-from-destination "~/jj-workspaces/repo/feature/")
                 "feature")))

(ert-deftest majutsu-workspace-add/interactive-only-prompts-for-destination ()
  "Interactive workspace add should derive name from destination."
  (let (seen-args)
    (cl-letf (((symbol-function 'majutsu-workspace--add-dir-default)
               (lambda () "/tmp/"))
              ((symbol-function 'read-directory-name)
               (lambda (&rest _args) "/tmp/feature"))
              ((symbol-function 'majutsu-read-string)
               (lambda (&rest _args)
                 (ert-fail "should not prompt for workspace name")))
              ((symbol-function 'majutsu-read-revset)
               (lambda (&rest _args)
                 (ert-fail "should not prompt for revision")))
              ((symbol-function 'majutsu-completing-read)
               (lambda (&rest _args)
                 (ert-fail "should not prompt for sparse patterns")))
              ((symbol-function 'make-directory)
               (lambda (&rest _args) nil))
              ((symbol-function 'majutsu-run-jj)
               (lambda (&rest args)
